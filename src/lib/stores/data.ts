...
const mapSetsDir = await path.join(home, "Stosufy/Songs");
const mapDataFile = await path.join(mapSetsDir, "songData.json");
export let mapDataStore = writable({});

if (
  (await exists(home + "/Stosufy", { baseDir: BaseDirectory.Home })) === false
) {
  await mkdir(home + "/Stosufy", { baseDir: BaseDirectory.Home });
}

if ((await exists(mapSetsDir, { baseDir: BaseDirectory.Home })) === false) {
  await mkdir(mapSetsDir, { baseDir: BaseDirectory.Home });
}

if (
  (await exists(mapDataFile, {
    baseDir: BaseDirectory.Home,
  })) === false
) {
  const file = await create(mapDataFile, {
    baseDir: BaseDirectory.Home,
  });
  ...
  const mapData = JSON.parse(
    await readTextFile(mapDataFile, {
      baseDir: BaseDirectory.Home,
    })
  );
...
export async function deleteSong(setId) {
  ...
  const deletedIndex = formattedSongs.findIndex(
    (song) => song.id.toString() === setId.toString()
  );
  ...
  if (
    audioFilePath &&
    (await exists(audioFilePath, { baseDir: BaseDirectory.Home }))
  ) {
...
export async function downloadBeatmap(
  ...
  const audioBuffer = await extractAudioFromBeatmap(
    buffer,
    (mapId = null),
    setId,
    mapSetData
...
async function extractAudioFromBeatmap(mapsetBuffer, mapId, setId, mapsetData) {
  try {
    ...
    const extractDir = await path.join(
      home + "/Stosufy/extract",
...
async function updateMapsetData(mapsetData) {
  ...
  if (mapsetData.beatmaps && Array.isArray(mapsetData.beatmaps)) {
    mapsetData.beatmaps.forEach((beatmap) => {
      ...
      const isDownloaded =
        updatedMapData[setId].beatmaps[currentMapId]?.downloaded || false;
